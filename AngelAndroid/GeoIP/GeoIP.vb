Public Class GeoIP
    ...
    Public Sub New(ByVal mss As IO.MemoryStream)
        MS = mss
    End Sub
    Public Sub New(ByVal FileLocationd As String)
        If System.IO.File.Exists(FileLocationd) Then
            Dim FS1 As New IO.FileStream(FileLocationd, IO.FileMode.Open, IO.FileAccess.Read)
            MS = New IO.MemoryStream()
            Dim BY(256) As Byte
            While FS1.Read(BY, 0, BY.Length) <> 0
                MS.Write(BY, 0, BY.Length)
            End While
            FS1.Close()
        End If
    End Sub
    Private Function ConvertIPAddressToNumber(ByVal IPA As Net.IPAddress) As Long
    ...
    Public Shared Function FileToMemory(ByVal FileLocation As String) As IO.MemoryStream
        Dim FS As New IO.FileStream(FileLocation, IO.FileMode.Open, IO.FileAccess.Read)
        Dim MES As New IO.MemoryStream()
        Dim BY(256) As Byte
...
End Module
 Public Module GetFlagThisIp
    Public Function ThisIp(ByVal ipc As String) As System.Drawing.Image
        ...
        i = Array.IndexOf(sockets.Data.GeoIP0.CountryCode, DOL)
        index_image = sockets.Data.imageFlags.Images.IndexOfKey(DOL)
        Select Case i
            Case -1
                i = 0
                index_image = -1
            Case sockets.Data.GeoIP0.CountryName.Length
                i = i - 1
                index_image = -1
            Case > sockets.Data.GeoIP0.CountryName.Length
                i = 0
